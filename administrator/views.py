...
def approveVoter(request,voter_id,admin_id):
    obj = Voter.objects.get(id=voter_id)
    admin = CustomUser.objects.get(id=admin_id)
    admin.approve_status = 1
    obj.approve_status = 1
    obj.save()
    admin.save()
    return HttpResponseRedirect(reverse('adminViewVoters'))

def disapproveVoter(request,voter_id, admin_id):
    obj = Voter.objects.get(id=voter_id)
    admin = CustomUser.objects.get(id=admin_id)
    admin.approve_status = 2
    obj.approve_status = 2
    obj.save()
    admin.save()
    return HttpResponseRedirect(reverse('adminViewVoters'))
